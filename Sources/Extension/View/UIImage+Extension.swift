...
extension UIImage {
    ...
    /// - Parameter color: 颜色值
    /// - Returns: 返回图片
    class func creatImg(color: UIColor) -> UIImage? {
    ...
    ///   - radius: 圆角尺寸
    /// - Returns: 处理后图片
    func roundImage(corners: UIRectCorner = UIRectCorner.allCorners, radius: CGSize) -> UIImage? {
    ...
    /// - Returns: 结果图片
    func waterMarkedImage(waterMarkText: String, corner: WaterMarkCorner = .BottomRight, margin: CGPoint = CGPoint(x: 20, y: 20), waterMarkTextColor: UIColor = .white, waterMarkTextFont: UIFont, backgroundColor:UIColor = .clear) -> UIImage {
    ...
    // 加载GIF图片
    static func gifImage(with data: Data) -> UIImage? {
        guard let source = CGImageSourceCreateWithData(data as CFData, nil) else {
            MyLog("Error: Could not create image source")
            return nil
        }
